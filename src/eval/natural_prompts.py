...
class NaturalPromptAgent:
    ...
    def __init__(self, inner: Agent, enabled: bool = True) -> None:
        self.inner = inner
    ...
    def respond(self, session_id: str, user_message: str, turn: int, top_k: int) -> dict:
        rewritten = naturalize(user_message) if self.enabled else user_message
        self.rewritten += int(rewritten != user_message)
        return self.inner.respond(session_id, rewritten, turn, top_k)


def run_benchmark(catalog: str, dataset: str, enabled: bool = True) -> dict:
    """Evaluate natural customer wording with unchanged targets and simulator.

    `enabled=False` is the NULL CONTROL: the proxy is still in the call path but
    rewrites nothing, so it must reproduce the official score exactly. Without
    that check a degraded number is ambiguous -- it could mean the agent is
    brittle, or merely that the wrapper itself perturbs something. Run the
    control before believing the diagnostic.
    """
    samples = load_jsonl(dataset)
    catalog_ids, categories, products = catalog_index(catalog)
    wrapper = NaturalPromptAgent(Agent(catalog), enabled=enabled)
...
def main() -> int:
    ...
    parser.add_argument("--control", action="store_true",
                        help="also run the null control (proxy in path, no rewriting); "
                             "it must reproduce the official score exactly")
    parser.add_argument("--catalog", default="data/catalog.jsonl")
    parser.add_argument("--dataset", default="data/public_set.jsonl")
    args = parser.parse_args()

    if args.control:
        control = run_benchmark(args.catalog, args.dataset, enabled=False)
        print("NULL CONTROL (proxy in path, no rewriting)")
        print(f"  rewritten turns        : {control['natural_prompt_diagnostic']['rewritten_customer_turns']} (expect 0)")
        print(f"  technical score        : {control['recommended_technical_score']:.6f}")
        print("  ^ must equal the official score, or the wrapper itself is perturbing the run\n")
